...
class FormElement extends HTMLElement {
	 ...
	 * @return string
	 */
    public static function getElement($data) { // todo: check default value
		$sId = $data['id'];
		...
		switch ($sType) {
			case 'text':		$sHTML .= self::getInputText($sId,$sValue,$aValues,$aAttr); break;
			case 'select':		$sHTML .= self::getSelect($sId,$sValue,$aValues,$aAttr); break;
			case 'checkbox':	$sHTML .= self::getCheckbox($sId,$sValue,$aValues,$aAttr); break;
			case 'textarea':	$sHTML .= self::getTextArea($sId,$sValue,$aAttr); break;
			case 'hidden':		$sHTML .= self::getHidden($sId,$sValue,$aAttr); break;
			case 'array':		$sHTML .= self::getArray($sId,$sValue); break;
			default:			$sHTML .= self::getInput($sType,$sId,$sValue,$aAttr);
		}
		/*
		 *  todo: add all types:
		 * 		progress
    ...
    public static function getInput($type,$id,$value='',$attr=array()) {
    ...
    public static function getInputText($id,$value,$values,$attr) {
		$sHTML = '';
		$attr['type'] = 'text';
		if (count($values)==0) {
    ...
    public static function getSelect($id,$value,$values,$attr=array()) {
		$sHTML = '';
    ...
    public static function getRadios($id,$value,$values,$attr=array()) {
		$sHTML = '';
		...
		foreach ($values as $sValueId=>$sValueLabel) {
			...
			if ($bChecked===false&&($value==''||$sValueId==$value)) {
				$bChecked = true;
				$aAttrSingle[] = 'checked';
			}
			if ($sValueId==$value) $aAttrSingle[] = 'checked';
			$sHTML .= self::get('li',array(),self::get('input',$aAttrSingle,$sValueLabel));
		}
		return self::get('ul',array(),$sHTML);
    }

	public static function getCheckbox($id,$value='',$values=array(),$attr=array()){ // todo: set checked status if true
		$sHTML = '';
		$attr['type'] = 'checkbox';
		if (count($values)==0) {
			$attr['id'] = $id;
	...
	public static function getTextArea($id,$value,$attr){
	...
	public static function getHidden($id,$value,$attr=array()){
		return self::get('input',array_merge($attr,array(
			'type'=>'hidden'
			,'id'=>$id
			,'name'=>$id
	...
	public static function getArray($id,$value){
		$sHTML = '<div class="settings-array">';
		$sHTML .= 	'<input name="'.$id.'" id="'.$id.'" type="hidden" value="'.str_replace('"','&quot;',$value).'" />';
		$sHTML .= 	'<table>';
